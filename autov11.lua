--[[ 
    FISH-IT BOT v29 (TIME SPOOF EDITION)
    Fitur Baru: 
    - Toggle "Time Spoof" di UI.
    - Jika ON: Kirim waktu palsu (Mundur seharga WaitGame) -> Finish Instan.
    - Jika OFF: Kirim waktu asli -> Menunggu WaitGame (Aman).
]]
...
local BotConfig = {
    ...
    UseTimeSpoof = false, -- Default mati biar aman
    
    WaitThrow = 0,
    WaitBite = 0,
    WaitGame = 2.5, -- Waktu yang akan di-spoof atau ditunggu
    WaitCool = 0.5,
...
MainFrame.Size = UDim2.new(0, 420, 0, 480) -- Sedikit lebih tinggi untuk toggle
...
TitleLabel.Text = "⚡ Fish-It v29 (Spoof Toggle)"
...
SettingFrame.Size = UDim2.new(1, -20, 0, 220) -- Diperbesar
...
-- SETUP INPUTS
CreateToggle("⚡ TIME SPOOFING", "UseTimeSpoof", BotConfig.UseTimeSpoof) -- FITUR BARU
...
LogFrame.Size = UDim2.new(1, -20, 1, -310) -- Disesuaikan tinggi setting
...
local function AddLog(text, colorType)
    ...
    if colorType == "debug" then color = Color3.fromRGB(255, 100, 255) end
...
local function StartBot()
    task.spawn(function()
        ...
        if BotConfig.UseTimeSpoof then
            AddLog("⚡ MODE: TIME SPOOF ON (RISKY)", "warn")
        else
            AddLog("🛡️ MODE: SAFE (Wait "..BotConfig.WaitGame.."s)", "info")
        end
        ...
        while BotConfig.IsRunning do
            ...
            repeat
                ...
                -- [[ LOGIKA TIME SPOOFING DI SINI ]] --
                local TimeToSend = os.time()
                if BotConfig.UseTimeSpoof then
                    -- Kalau Spoof ON: Kita kurangi waktu sekarang dengan WaitGame
                    -- Server mengira kita sudah main dari tadi
            ...
            -- 3. EKSEKUSI
            if Ticket then
                -- [[ LOGIKA MENUNGGU DI SINI ]] --
                if BotConfig.UseTimeSpoof then
                    -- Kalau Spoof ON, kita LANGSUNG finish (karena waktu sudah dimanipulasi di atas)
                    AddLog("⚡ Instant Finish (Spoof)!", "success")
                    task.wait(0.1) 
                else
                    -- Kalau Spoof OFF, kita WAJIB menunggu waktu asli
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        MainFrame:TweenSize(UDim2.new(0, 420, 0, 480), "Out", "Quad", 0.3, true)
