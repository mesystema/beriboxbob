--[[ 
    FISH-IT BOT v27 (SEQUENTIAL FLOW CONTROL)
    Fokus: MENGHILANGKAN DUPLIKAT AKSI
    
    Perubahan Logika:
    - Menghapus 'task.spawn' pada aksi utama (Wajib antri/sekuensial).
    - Menambahkan 'WaitThrow' di UI (Waktu tunggu setelah lempar sebelum pelampung dianggap jatuh).
    - Alur: Lempar -> Wait -> Gigit -> Wait -> Minigame -> Wait -> Tarik -> Wait.
]]
...
local BotConfig = {
    ...
    -- 1. Jeda setelah Melempar (Menunggu pelampung stabil di air)
    WaitThrow = 1.0, 
    
    -- 2. Jeda Menunggu Ikan Menggigit
    WaitBite = 2.0, 
    
    -- 3. Durasi Main Minigame
    WaitGame = 2.6, 
    
    -- 4. Jeda Setelah Dapat Ikan (Sebelum mulai lagi)
    WaitCool = 1.0, 
...
-- CLEANUP
if CoreGui:FindFirstChild("FishBotUI_V27") then CoreGui.FishBotUI_V27:Destroy() end

local Remotes = { Charge = nil, Minigame = nil, Finish = nil }

-- [[ UI CREATION ]] --
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FishBotUI_V27"
...
MainFrame.Size = UDim2.new(0, 420, 0, 450) -- Diperbesar untuk muat banyak setting
...
TitleLabel.Text = "📝 Fish-It v27 (Sequential)"
...
SettingFrame.Size = UDim2.new(1, -20, 0, 180) -- Area input lebih besar
...
-- BUAT INPUT SESUAI REQUEST
CreateInput("1. Jeda Lempar (WaitThrow)", "WaitThrow", BotConfig.WaitThrow)
CreateInput("2. Jeda Gigit (WaitBite)", "WaitBite", BotConfig.WaitBite)
CreateInput("3. Lama Minigame (WaitGame)", "WaitGame", BotConfig.WaitGame)
CreateInput("4. Jeda Selesai (WaitCool)", "WaitCool", BotConfig.WaitCool)
