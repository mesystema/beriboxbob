--[[ 
    FISH-IT BOT v21 (PROTOCOL BREAKER)
    Fokus: MENGABAIKAN RESPON SERVER (INSTANT)
    
    Teknik:
    - Mengubah InvokeServer menjadi "Fire & Forget" menggunakan task.spawn.
    - Script TIDAK AKAN MENUNGGU pelampung jatuh.
    - Script langsung menembak remote Minigame 0.05 detik setelah lempar.
]]
...
local BotConfig = {
    IsRunning = false,
    -- Jeda antar paket data (Sangat cepat)
    -- Jika terlalu cepat (0.00), server mungkin menerima urutan terbalik (Minigame dulu baru Lempar = Error)
    PacketDelay = 0.05, 
    
    WaitGame = 2.6, -- Waktu Minigame (Tetap harus dihargai agar tidak kick)
    WaitCool = 0.1,
    CastPower = 1.0
}
...
if CoreGui:FindFirstChild("FishBotUI_V21") then CoreGui.FishBotUI_V21:Destroy() end
local Remotes = { Charge = nil, Minigame = nil, Finish = nil }

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FishBotUI_V21"
...
MainFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0) -- Hitam Pekat
...
TitleLabel.Text = "⚡ PROTOCOL BREAKER v21"
TitleLabel.TextColor3 = Color3.fromRGB(255, 0, 0) -- Merah Error
...
local function StartBot()
    task.spawn(function()
        ...
        AddLog("⚠️ PROTOCOL BREAKER ON", "warn")

        while BotConfig.IsRunning do
            local success, err = pcall(function()
                
                KillAnimations() -- Hapus semua animasi

                -- 1. ASYNC CHARGE (LEMPAR)
                -- Kita pakai task.spawn agar TIDAK MENUNGGU server
                task.spawn(function()
                    Remotes.Charge:InvokeServer(workspace.DistributedGameTime)
                end)
                
                -- Kita tidak menunggu jawaban Charge!
                -- Kita hanya memberi jeda 0.05 detik (50 milidetik) agar urutan paket data tidak tertukar
                task.wait(BotConfig.PacketDelay)

                -- 2. ASYNC MINIGAME (MINTA IKAN)
                -- Kita berasumsi Charge sudah dikirim, langsung minta minigame
                local randomID = math.random(100000000, 999999999) 
                
                -- Kita coba InvokeServer biasa di sini karena kita butuh jeda minigame
                Remotes.Minigame:InvokeServer(BotConfig.CastPower, randomID, os.time())
                
                -- Tunggu Server (Minigame Time)
                -- Ini satu-satunya jeda yang wajib
                AddLog("⏳ Server Check ("..BotConfig.WaitGame.."s)...", "info")
                ...
                AddLog("✅ CYCLE SELESAI", "success")
                
                -- Langsung ulang tanpa cooldown
